...
| `algos/rainbow/net.py`, `noisy.py` | `RainbowNet`: a trunk (`QNet`'s stack, or BTR's residual MLP with spectral norm inside the blocks) under dueling value/advantage streams that are `NoisyLinear` (factorised Gaussian, `active` by name, not by mode) or plain, read as c51, quantile or IQN in the shapes `DistAgent` calls; the sidecar's `trunk` field says which. The greedy read has the noise off |
| `algos/rainbow/agent.py`, `algo.py` + `rainbow.py`, `btr.py` | `RainbowAgent`: `DistAgent`'s losses, acting with the noise **on**, double-Q or the target's own argmax; `RainbowAlgo`: `DqnAlgo` with the papers' defaults per name, the noisy flag lifting the epsilon floor, and `SNEK_RAINBOW_EPSILON_ZERO_AT`. `algos/dqn/` and `algos/dist/` are imported, never edited (`plans/algoExploration/c-value-stack.md`) |
