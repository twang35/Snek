...
Three measurements shape everything here, taken with a 98%-perfect checkpoint:

| what | number |
|---|---|
| policy inference, batch 512 | 1.5M rows/s |
| env step **without** the observation, n=512 | 211 us |
| env step **with** it, n=512 | 4,323 us |

So **the observation is 95% of a step and the policy is nearly free.** Two consequences drive the
design. The env step gets cheaper per row as the batch widens — 55k steps/s at n=128, 174k at n=2048
— so width is the lever; and since the policy is cheap, one wide env can serve *several checkpoints
at once*, each with its own policy call on its own slice.

**The framework seam is `policy_fn`**, one callable of shape `(m, OBS_LEN) float32 -> (m,) int`.
Nothing here imports torch, which is what lets the engine be tested and benchmarked against a
...
finishes early, so the ones still playing are the good ones. Here a checkpoint has exactly
`episodes` episodes **started** on its behalf, and every started episode is run to completion, so no
episode is ever selected on its length. Which physical lane runs one is irrelevant — episodes are
...
def measure_stream(next_job, on_complete, episodes, width=None, max_live=None, seed=0,
    ...
    actions. Keeping it a plain callable is deliberate: this module imports no torch, so it can
    ...
    def assign(rows, already_fresh=False):
        ...
        `tests/test_engine.py` comparing the engine's returns against a replay.
        """
        for row in rows:
            job = take_job()
            if job is None:
