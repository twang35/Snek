...
Parity target is `env/` itself: `env.game.Game.step` for the mechanics and rewards, and
`env.observations.get_observations` for the 30-value observation. `tests/test_vec_parity.py`
asserts both elementwise against a live `Game`, so this file is a *reimplementation*, never an
approximation — if it and `env/` disagree, this file is wrong.
...
---------------------------------------------------
The same reason `env.observations.count_groups` uses one: a round costs a handful of operations
...
wall clock, because a round was already nearly free. Batching is what pays here.
"""
...
# against it in tests/test_vec_config.py rather than trusted.
...
def _flood(seed, open_, max_iter=4 * NCELL):
    ...
    **Compaction is not an optimisation here, it is the difference between this being worth doing and
    ...
    `tests/test_vec_env.py` asserts the compacted and naive paths agree, because a bug here
...
def _enumerate(pk, head_nb, tail_nb, food_bit, max_regions=NCELL):
    ...
    the other is exactly the trap the flag exists to name. See `env.observations.group_obs`.
    """
    M = pk.shape[0]
...
class VecSnake:
    ...
    def step(self, actions, forced_food=None, autoreset=True, observe=True, groups_mode='full'):
        ...
        rule this project has paid for twice over (`env.observations.is_perfect_score`).
    ...
    def _chase_safe_now(self):
        """`env.observations.chase_safe_state`, vectorised: head, food and tail in *one* region.
    ...
    def _free_space_now(self):
        """`env.observations.free_space_pieces`, vectorised: 1 / open-region count, tail cell freed.
