...
from env import constants as sc
from env import observations as state_helpers
from env.game import Game, GameSnapshot, direction_between
...
def _with_shaping(names, run_kwargs, states=4000):
    ...
    Both modules have to be patched, and the reason is a real trap rather than caution: `env.game`
    does `from env.constants import *`, so it holds its *own* bound copies of these four names.
    Setting them on `env.constants` alone would leave the reference reading the old values and the
    test would pass while comparing nothing.
    ...
    """
    from env import game as reference
    saved = {}
    try:
        for module in (reference, C):
