...
def test_observation_spec_matches_what_the_game_emits():
    ...
    snek2's `head_with_food_obs` sat at 0 in the spec for a whole signature generation while the
    argument it needed was still being passed, and the length is a hand-maintained sum of named
    blocks, so it drifts silently. This asserts the block table, the spec and the vector the game
    actually emits all agree, rather than trusting any one of them.

    The literal is a deliberate tripwire, not redundancy with the equalities: adding a block is
    supposed to fail here so `OBS_LEN`, `OBS_BLOCKS`, the layout table in `docs/environment.md` and
    `OBS_ERA` all get updated in the same pass.
    """
    from env import scalar_env

    env = scalar_env.SnakeEnv()
    observation = env.reset()
    assert scalar_env.observation_length() == 30
    assert env.observation_spec()['shape'] == (30,)
    assert observation.shape == (30,)


def test_done_is_true_exactly_on_the_terminal_step():
    """`done` is the bootstrap mask, and it is the env's whole statement about termination.

    snek2 returned a TimeStep whose terminal `discount` had to be zeroed by hand, and for a long
    time it was not: every episode's final transition trained toward `reward + 0.9975 *
    V(terminal)` instead of toward `reward`, discounting the death penalty by whatever value the
    network happened to assign a terminal observation. `SnakeEnv` returns no discount field at all,
    so this flag is the only thing an algorithm's `(1 - done)` factor can rest on.
    """
    from env import scalar_env

    env = scalar_env.SnakeEnv(discount=0.9975)
    env.reset()
    flags = []
    # Straight ahead from the start tile walks into a wall inside ten steps, and a length-4 snake
    # cannot starve in that time, so this terminates by death.
    for _ in range(50):
        _, _, done, info = env.step(ACTIONS.index('forward'))
        flags.append(done)
        if done:
            assert info['done'] is True
            assert info['died'], 'walking into a wall should be a death, got {0}'.format(info)
            break
    assert flags and flags[-1] is True, 'the episode never terminated'
    assert not any(flags[:-1]), 'done was raised before the episode ended'
