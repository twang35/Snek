"""Tests for `vectorized/engine.py` — the batched measurement loop.

The env is covered by `test_vec_parity.py` and `test_vec_env.py`; what is left here is
...
The policies here are hand-written, so none of this needs torch.
"""
...
def test_the_single_checkpoint_helper_agrees_with_the_stream():
    ...
    """
    held = engine.measure(survival_policy, 40, lanes=20, seed=2)
...
def test_max_live_is_derived_with_enough_slack_to_keep_the_batch_full():
    for width, episodes in ((1024, 100), (1024, 500), (4000, 500), (512, 20)):
        live = engine.default_max_live(width, episodes)
        assert live * episodes > width, (width, episodes, live)
        assert live >= engine.MIN_MAX_LIVE
...
def test_a_job_banks_each_episode_at_its_start_slot_not_its_completion_order():
    ...
    """
    job = engine._Job('ckpt', lambda obs: obs, 4)
...
def test_banking_one_slot_twice_is_refused():
    job = engine._Job('ckpt', lambda obs: obs, 2)
...
def test_a_partial_job_refuses_to_hand_out_a_sample():
    ...
    job = engine._Job('ckpt', lambda obs: obs, 3)
    job.record(0, 95, 95.0)
    try:
        job.held()
    except RuntimeError as error:
