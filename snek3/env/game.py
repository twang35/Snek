...
class SnakeSegment(pygame.sprite.Sprite):
    def __init__(self, tile_pos, segment_groups, color=R.SNAKE_SEGMENT_COLOR, radius=R.SNAKE_SEGMENT_RADIUS):
        pygame.sprite.Sprite.__init__(self)
        self.image = self.image = pygame.Surface(R.TILE_SIZE).convert()
        self.image.fill(R.COLORKEY_COLOR)
        self.image.set_colorkey(R.COLORKEY_COLOR)
        pygame.draw.circle(self.image, color, R.TILE_RECT.center, radius)

        self.tile_pos = tile_pos

        self.rect = self.image.get_rect()
        self.rect.topleft = (tile_pos[0] * R.TILE_SIZE[0], tile_pos[1] * R.TILE_SIZE[1])
    ...
    def move(self):
        ...
        self.rect.move_ip(R.MOVE_VECTORS_PIXELS[self.move_dir])
...
class SnakeHead(SnakeSegment):
    def __init__(self, tile_pos, move_dir, segment_groups):
        SnakeSegment.__init__(self, tile_pos, segment_groups, color=R.SNAKE_HEAD_COLOR, radius=R.SNAKE_HEAD_RADIUS)
...
class Food(pygame.sprite.Sprite):
    def __init__(self, taken_up_group, position=None):
        ...
        self.image.fill(R.COLORKEY_COLOR)
        self.image.set_colorkey(R.COLORKEY_COLOR)
        pygame.draw.circle(self.image, R.FOOD_COLOR, R.TILE_RECT.center, R.FOOD_RADIUS)
        ...
        if position is not None:
            ...
            self.rect.topleft = (self.position[0] * R.TILE_SIZE[0], self.position[1] * R.TILE_SIZE[1])
            return
        while True:
            self.position = (
            ...
            self.rect.topleft = (
                self.position[0] * R.TILE_SIZE[0],
                self.position[1] * R.TILE_SIZE[1]
            )
            continue_loop = False
...
class Game:
    def __init__(self, display=True, limit_fps=False, policy_name='', discount=1.0):
        ...
        self.caption = '{0} — {1}'.format(policy_name, R.CAPTION) if policy_name else R.CAPTION
    ...
    def reset(self):
        ...
        self.bg = pygame.Surface(R.SCREENSIZE).convert()
        self.bg.fill(R.BACKGROUND_COLOR)
    ...
    def _rebuild_grid(self):
        ...
        lookup offsets by +1 — see get_grid_value() in env.observations.
        ...
        """
        column = R.SCREENSIZE[0] // R.TILE_SIZE[0]
        row = R.SCREENSIZE[1] // R.TILE_SIZE[1]
    ...
    def step(self, relative_direction):
        ...
        if ate_food:
            ...
            if self.check_perfect_game():
                ...
                reward = constants.PERFECT_GAME_REWARD
        ...
        if pos[0] < 0:
            self.finished = True
            reward = DEATH_REWARD
        if pos[0] >= R.SCREENSIZE[0]:
            self.finished = True
            reward = DEATH_REWARD
        if pos[1] < 0:
            self.finished = True
            reward = DEATH_REWARD
        if pos[1] >= R.SCREENSIZE[1]:
            self.finished = True
            reward = DEATH_REWARD
    ...
    def _hud_font(self):
        return self._font(R.HUD_FONT_SIZE)
    ...
    def render(self):
        if self.perfect_game and constants.DEBUG_LOGGING:
        ...
        if self.perfect_game:
            f = self._message_font('PERFECT GAME!!!', R.PERFECT_FONT_SIZE)
            fail_message = f.render('PERFECT GAME!!!', True, (0, 0, 0))
            fail_rect = fail_message.get_rect()
            fail_rect.center = R.SCREENRECT.center
            self.screen.blit(fail_message, fail_rect)
            pygame.display.flip()
            pygame.time.wait(R.PERFECT_GAME_WAIT_MS)
            return

        if self.finished:
            if self.starved:
                death_reason = 'NO FUD'
                f = self._message_font(death_reason, R.STARVE_FONT_SIZE)
            else:
                death_reason = 'DED'
                f = self._message_font(death_reason, R.DEATH_FONT_SIZE)
        ...
        # score
        f = self._hud_font()
        d = self.screen.blit(self.bg, R.SCORE_POS, pygame.Rect(R.SCORE_POS, R.HUD_ERASE_SIZE))
        score_image = f.render(R.SCORE_PREFIX + str(self.current_score), True, R.SCORE_COLOR)
        d2 = self.screen.blit(score_image, R.SCORE_POS)
        # steps
        d3 = self.screen.blit(self.bg, R.STEP_POS, pygame.Rect(R.STEP_POS, R.HUD_ERASE_SIZE))
        step_image = f.render(R.STEP_PREFIX + str(self.current_step), True, R.STEP_COLOR)
        d4 = self.screen.blit(step_image, R.STEP_POS)
        # policy name
        d5 = self.screen.blit(self.bg, R.POLICY_POS, pygame.Rect(R.POLICY_POS, R.HUD_ERASE_SIZE))
        policy_image = f.render(R.POLICY_PREFIX + self.policy_name, True, R.POLICY_COLOR)
        d6 = self.screen.blit(policy_image, R.POLICY_POS)
        ...
        if self.limit_fps:
            # Read through the module, not the star-imported name: `from env.constants import
            # *` binds a copy at import time, so watch.py setting R.FPS_LIMIT
            # would have had no effect here. PERFECT_GAME_WAIT_MS above is qualified for the
            # same reason.
            self.clock.tick(R.FPS_LIMIT)

        # slow down when close to finished
        if self.current_score >= R.SCORE_SLOW_THRESHOLD:
            self.clock.tick(R.SCORE_THRESHOLD_FPS)
