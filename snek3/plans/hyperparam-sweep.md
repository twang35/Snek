...
| order | batch | knob | what it is | waves | ~time | state (2026-09-04 23:30) |
|---:|---|---|---|---:|---:|---|
...
| 5 | b13 | minibatch | gradient batch size, and steps per epoch | 4 | laptop, ~11 h | closed 09-04 ~10:30; hof passes done on the desktop |
| 6 | b14 | rollout T | steps per lane before an update | 3 | laptop, 11 h + 1.6 h hof | closed 09-04 21:37, hof passes 23:14 (T 512/1024 waves ran in ~1 h each) |
...
**One queue for both boxes since 2026-09-05 23:43** (`plans/archive/shared-queue.md`): nothing is assigned to a box any more. Each box's scheduler claims the next wave of the highest-priority batch when it comes free, so a batch spans both boxes wave by wave -- b20 ran w1 on the laptop and w2 on the desktop, b21 w1 and w3 on the desktop and w2 on the laptop -- and `priority` orders the one pool (b21 at 220 went before b18 at 240). As of 2026-09-06 08:00 the desktop is on b21's last wave and the laptop on b18's first, with 16 b18 arms unclaimed; `tools.batch_state <batch>` prints a batch wave by wave with its boxes, and `status.json`'s `at_a_glance.pool` the unclaimed work. Laptop waves cost ~3.5 h each because stage B and the hof passes run in
