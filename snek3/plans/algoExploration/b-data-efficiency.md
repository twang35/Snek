...
### B1 -- BBF (Schwarzer et al. 2023, "Bigger, Better, Faster")
...
| n-step and γ anneal | 10 → 3 and 0.97 → 0.997, exponential, over 10k gradient steps after each reset | `algos/bbf/schedules.py`: from `SNEK_BBF_N_STEP_START` (10) to `SNEK_N_STEP_UPDATE` (3) and from `SNEK_BBF_GAMMA_START` (0.97) to `SNEK_DISCOUNT` (**0.997**, the paper's -- `README.md`, "Translating") over `SNEK_BBF_ANNEAL_STEPS` (10k gradient steps) after each reset. **`algos/dqn/collect.py`'s n-step window becomes settable between steps**; that is the one change to `algos/dqn/`, and it is a setter with a fixture that the window at n is exactly what the constant n produced |
| SPR | weight 5, 5-step latent prediction | **dropped for the first batch.** The self-predictive loss is a representation learner for pixels; on a 26-value vector it is a transition-model auxiliary of unclear value, and it is the one piece with its own network and its own knobs. `SNEK_BBF_SPR` is reserved and a second batch adds it at the paper's weight if B1 is short of the paper's relative gain. This is B1's one stated departure from the paper |
| dueling, double-Q, EMA target, weight decay | on, on, τ 0.005 every update with target-net action selection, AdamW 0.1 | `DuelingTrunk` is **Group D's** (`algos/rainbow/net.py`, decided 2026-09-20, `d-value-stack.md`); B1 imports its scalar form, so D's `net.py` lands before B1 is built -- the code, not D's batches; double-Q is DQN's; `SNEK_TARGET_UPDATE_TAU` 0.005 with period 1; AdamW `SNEK_BBF_WEIGHT_DECAY` 0.1 |
...
| head | C51, 51 atoms | **C51, 51 atoms on [−10, 110]** -- B1 runs in phase 1 beside A1, so the head is built here (`algos/dist/heads.Categorical`, A2's module, written early) and the support is the return-range choice of `a-return-tail.md` §1, not yet the one A2's stability batch settles on; if A2 later moves it, B1's local cell is re-run at A2's. A scalar-head cell reads against A1 without the distribution |
...
## 4. What B1's number is used for

The series budgets every row at the reference's 100M-transition cap because that is what PPO needed.
B1's onset step against A1's is the measurement that either confirms that or lets every later
value-based row (D, E2, F) queue at a shorter cap with a stated basis. The rule the plan proposes:
**if B1 reaches A1's cap-density within a fifth of A1's steps, the later value rows queue at half the
...
- **B1 matches A1's density at the short cap.** The rule in §4 fires. Also: the resets are worth
...
- **B1 trails A1 at the short cap but wins the long one.** The resets are a late-stability lever, not
  an efficiency one, on this game; the long batch's drawdown count is the evidence.
- **B1 is below A1 everywhere.** The 100k-regime recipe does not transfer off pixels; SPR is tried once
