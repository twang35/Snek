...
**Status: built 2026-09-20, reworked 2026-09-23 after an external review (the table below); queued 2026-09-23 as b46 (C1), b47 (C2), b48 (C1 ablation, laptop) and b49 (C2 ablation)**, paper and local cells sharing the step cap and eval every 1,000 (C1 3M steps, C2 1M = 64M moves in the paper cell), with no tuning wave: the paper cells are the paper's and the ablations flip flags on them (the user's call, 2026-09-23) (`algos/rainbow/`: `noisy.py`, `net.py`, `agent.py`, `algo.py`, the names `rainbow` and `btr` in
