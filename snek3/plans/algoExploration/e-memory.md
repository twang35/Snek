...
| training | the collectors own their own hidden states in the same way; PPO's `(T, N)` rollout stores the state at each step's start (E1) and the replay stores it per sequence (E2) |
...
- **Neither moves.** The 26 features plus eight moves are sufficient statistics for this policy class,
  the failures are not a memory problem, and the plan for F notes that its base (E2) is a null.
