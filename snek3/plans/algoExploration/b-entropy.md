...
|---|---|
| package | `algos/sac/`, one `algo.py` with `NAME`s `sac` and `sac2`, the second being the first with three knobs at non-default values (§2, B2). One package because B2 *is* B1 plus fixes, and the fixes are flags |
...
### B2 -- Revisiting Discrete SAC (Zhou, Wang, Feng & Zhou 2022; TMLR 2024, arXiv 2209.10081)
...
|---|---|---|---|
| optimiser, lr | Adam 3e-4, all nets | Adam 1e-5 actor and critic, 3e-4 for α when auto | the paper's, per row: B1 3e-4, B2 1e-5 |
| batch | 64 | 64 | 64 |
| replay | 1M; 20k random steps before learning | 1e5 | B1 1M, B2 1e5; prefill 20k moves |
| target | hard copy, "fixed network update frequency 8000" -- **the unit is not stated**; read as agent steps, as Dopamine counts it, that is 2,000 gradient updates at one update per 4 steps (the pseudocode writes Polyak, no τ given) | Polyak τ 0.005 | `SNEK_SAC_TARGET_UPDATE_PERIOD` **2,000** gradient updates for B1 (the knob counts updates, as DQN's does); τ 0.005 for B2 |
| update frequency | 1 gradient step per 4 env steps | 0.1 per step (Tianshou `update-per-step`) | `SNEK_SAC_REPLAY_RATIO` 0.25 for B1, 0.1 for B2 |
| n-step | 1 | 3 | per row |
...
if B1 and B2 are level there is nothing to attribute.
...
4. Tuning budget: one laptop wave over α (§2b) for B2 and over `SNEK_SAC_LEARNING_RATE` for B1.

## 5. What would change the plan

- **B1 matches PPO.** Entropy regularisation, not the on-policy update, is what PPO's result rests on.
  That makes A6's Munchausen result (an entropy-regularised value target) the natural comparison, and
  argues for adding a tuned-temperature entropy term to the value rows in C and E.
- **B1 collapses and B2 does not.** The implementation, not the idea; the halves say which fix. This
