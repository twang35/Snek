...
| the batch | 4 seeds a cell, seed pinned to the arm letter, the current PPO reference config for everything the algorithm does not own (reward preset, `SNEK_OBS_HISTORY`, and `SNEK_FC_LAYERS` as the analogue of the paper's convolutional encoder -- every layer the paper has after it keeps the paper's width, next section), stage A at 100 episodes on every checkpoint, stage B, `hof5000`, `hof30k`. Judged on stage-B density, the depth passes and the drawdown count, never on a single eval | `docs/protocol.md` |
| **paper fidelity** | **every row's first cell is the paper's configuration wherever the game allows** (widened 2026-09-23, the user's rule), translated to Snake by the rules in the next section and nothing else -- the paper's loss **and its reduction**, priorities and importance-weight normalisation, gradient clipping, optimiser with its ε and betas, learning rate, batch, replay size and prefill policy, n-step, target period, exploration schedule **and its shape**, and every layer width that has a counterpart (a dueling stream, a dense layer after the encoder, an LSTM), with this codebase's own additions (the fork, the shield, the eval-driven epsilon, the horizon anneal, mean-normalised importance weights) **off**. Where the paper is silent, its reference code; where paper and code disagree, the plan says which and why (C2 takes BTR's code, which produced its numbers). **A shared implementation that departs from a paper is not a reason for the paper cell to depart**: the row adds a knob with the paper's form as its default (`c-value-stack.md`, decided 2026-09-23, is the worked example). A second cell, the **local** cell, runs the same algorithm on the codebase's tuned defaults where the two differ materially; it is what says whether a gap is the algorithm or the plumbing. The paper cell is the headline, the row's plan carries a "paper settings" table naming each knob's paper value, its translation and the reason where one does not transfer, and a spec that departs from the table says so in its manifest note | each plan's "paper settings" section; `docs/protocol.md` |
...
| **the reward preset and the observation** | **`hist8` is the game**: every cell, paper and local, sees the 26-value observation with 8 frames of history. **The potential shaping is not**: paper cells run `SNEK_CHASE_SAFE_SHAPING=0` (the b2 preset's food 1, death −5, win +100 and step penalty 0.01 stay); local cells keep the reference's 0.1 at gate 75 | decided 2026-09-23 by the user: the history is what the agent observes, so it is the environment; the shaping is a reward term this codebase added to help learning, so it changes the objective the paper optimises. BBF's paper cell (b44) already ran it off |
...
| **target network period** | the paper's, **in gradient updates**, converted once where the paper counts agent steps (Rainbow's 8,000 agent steps at one update per 4 is 2,000 updates; `a-return-tail.md` §1b) | the codebase's `SNEK_TARGET_UPDATE_PERIOD` default of 8 is a hard copy every 8 updates and is the single largest departure from every value paper; it stays in the local cell only |
...
| **a CNN trunk (Nature DQN, IMPALA, ResNet)** | **the convolutional stack** becomes the reference's `fc 320` MLP over the 26+16-value observation (or the MLP analogue a plan states, e.g. C2's residual stack); **every dense, stream, embedding-product or recurrent layer after it keeps the paper's width** (Nature's 512, Rainbow's and BTR's 512 streams, IQN's 512 after the embedding) | there is no image, so only the part that reads pixels is substituted; the rest of the network has a counterpart and is the paper's (2026-09-23) |
| **an LSTM of 512 (R2D2) or a 256-wide recurrent core** | **the paper's width** (2026-09-23; it was "where the trunk is comparable") | the core sits after the encoder, so it has a counterpart; a smaller core is a local-cell question |
