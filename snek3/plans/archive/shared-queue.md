...
| a released number is never reused (decision 6) | **not as first built** (2026-09-06 audit): `release` deleted the file, so a release of the batch's highest wave handed the same number to the next claimer -- a `b21-stageb-w2` on each feed for two different waves. Fixed the same day: `release` rewrites a wave's file as a `released` tombstone (no arms, the number kept), `claimed_ids` frees the arms, and the numbering counts tombstones. An eval claim is still deleted, since nothing numbers it |
| the pool line `needs checkpoints` for an eval spec claimable by neither box (decision 7) | **not built**: the line reads `<batch> eval | <id> unclaimed` on both boxes; the `queue-batch` skill says what that means and how to check |
