...
| `.gitignore:81-87`; `snek3/CLAUDE.md` rows L35 (viewer layout), L213 (docs table), L238 (tools table); `docs/runs.md:114-116` (the TODO this supersedes); `plans/archive/sweep-analysis.md` (status line at the top: renamed and extended, date) | updated |
...
Entries to write, from `docs/results.md`, `plans/archive/hyperparam-sweep.md` and the ops specs:
...
| `docs/sweep.md`, `docs/runs.md`, `snek3/CLAUDE.md`, `plans/archive/sweep-analysis.md`, `.gitignore`, `skills/queue-batch/SKILL.md` | names, the TODO, the `describe` step |
...
- b1-b3 are included as browsable batches with degraded views, since the user asked for all; the alternative
  (leave them to the chart viewer, as `plans/archive/sweep-analysis.md:253` recommended) is a one-line removal from the
  batch list.
