...
|---|---|---|---:|---:|---|---|
| [b45](#b45--quantile-reads-acting-on-the-return-distribution-other-than-by-its-mean) | **the read, not the training**: sixteen acting rules over the same frozen weights (`--policy-variant`: `mean:fixed` control, `leastneg[:-2]` and `leastnegmean[:-2]`, `mix:0.7/0.5/0.3`, `mixmass:0.7`, `above:10/30/60`, `abovemean:10/30`, `cvar:0.25/0.5`) | the top 25 stage-A checkpoints of each arm of b37's C51 and QR-DQN cells, b38's IQN, b39's FQF and b40's M-QR-DQN | 5 cells × 16 reads × 100 checkpoints | 1,000 episodes a checkpoint, no stop rule | registered | — |
...
| | |
|---|---|
| base | frozen checkpoints, no training: the top 25 stage-A rows of each arm (`topa:25`) of `b37a`-`d` (C51), `b37e`-`h` (QR-DQN N 32), `b38a`-`d` (IQN N 8), `b39a`-`d` (FQF N 8) and `b40e`-`h` (M-QR-DQN), 100 a cell, 500 in all |
| varies | the acting rule over the head's `(values, masses)` (`algos/dist/reads.py`, `plans/quantile-reads.md` §1): `mean:fixed` (the control, IQN at fixed fractions), `leastneg` and `leastneg:-2` (the losses' share of the mean closest to 0, ties by the gains' share), `leastnegmean` and `leastnegmean:-2` (the same on conditional means), `mix:0.7` / `0.5` / `0.3` (a weighted mix of the negative and positive conditional means), `mixmass:0.7` (the same on partial expectations), `above:10` / `30` / `60` (the share of the mean above a threshold, the mean where no action has any), `abovemean:10` / `30` (the conditional mean above it), `cvar:0.25` / `0.5` |
| cells × seeds | 5 cells × 16 reads, each read one `eval` spec over the cell's four arms; 80 specs, the five controls first |
| cap | 1,000 episodes a checkpoint, every read, seed 0, no early stop; paired against the control by checkpoint (`tools/read_compare.py`) |
...
(`--policy-variant`, `a-return-tail.md` §5). A pilot on two b40e checkpoints (`plans/quantile-reads.md` §4b) already showed the
shape: the upward reads walk into the wall, because the never-taken action's quantiles are noise and some of that noise is large.
Queued 2026-09-22 on the desktop, all five cells' checkpoints on that box (b40's rsynced). The rows now carry `deaths` and `starves`
from the env's flags, added for this batch. No charts: the batch trains nothing; its result is `tools/read_compare.py`'s table per cell.
