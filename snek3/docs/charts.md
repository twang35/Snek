...
## Batch b11 — the `learning_rate` sweep, 2 values x 4 seeds, 50M (8 of 32 arms in)

**In flight: 8 of 32 arms trained, 0 running, 24 queued; stage B 0 wave(s) done; 3 training wave(s) left at ~2.5 h each -> ~2026-09-04 01:54** as of 2026-09-03 18:24. Reference group marked. Regenerated by `tools/progress_update.py`; only the reading block is hand-written.
...
| 1e-4 | 3,361 | 27.3% | 17.5 13.0 43.7 – | 122 | 100.0 | 98.17 (97.8-98.6) | 85.1 | 1.41% | 9.49% | 24.8% |
| **3e-4** (reference) | 5,173 | 27.3% | 27.2 24.1 31.4 26.0 | 138 | 100.0 | 98.33 (98.3-98.4) | 90.6 | 0.77% | 6.43% | 28.9% |

<!-- reading -->
**Wave 1 (lr 4e-5, 1e-4) trained to 50M by 17:42 on 2026-09-03 and its stage B is running**, so the
rows column is empty and the stage-B panels below are the box's in-progress pictures. Read the two cells
against the reference row, b9's λ 0.99 arms at lr 3e-4: lr 4e-5 finished at best30 97.0-97.6 with `sef`
65-79, the slow onset the spec predicted, and its stage-A traces are still climbing at the cap — it has
not reported its best. lr 1e-4 reached 97.8-98.6, inside the reference's 98.3-98.4 range on two of four
seeds, at a lower `sef` (85 against 91) because it spent longer getting there. The question wave 1's stage B
answers is whether the slower steps buy record density: the reference has 27.3%. Waves 2-4 (1.5e-4 through
2e-3) fill the grid above the base.
<!-- /reading -->
...
**Closed: all 64 arms trained, stage B done for every wave** as of 2026-09-03 18:24. Reference group marked. Regenerated by `tools/progress_update.py`; only the reading block is hand-written.
