...
| [`distributional-c51.md`](distributional-c51.md) | **proposed 2026-08-15. Phase 0 done** — the support is measured (`v_min -6`, `v_max 110`, 51 atoms) and the premise it was chosen against turned out to be wrong; control moved to `b25`. Phase 1 held pending review, phases 3-4 held until the desktop's shaping queue closes. A feasibility probe against this repo's own specs is also done, and is what fixes the two design choices a naive port would get wrong (IS weights are silently dropped by `tf_agents`' C51 loss; the PER priority has to be the KL, not the cross-entropy) |
