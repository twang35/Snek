...
|---|---|---|
| **Vectorised env + batched eval** | close-out and HOF latency, which is the thing blocking every launch | **approved 2026-08-23, in progress** — [plan](../plans/vectorized-eval.md). Eval engine only, in `snek2/vectorized/`; TF stays for inference because batched TF is 3.3M rows/s against the 176k/s the env can consume. Parity is deductive (bit-identical observations under forced food ⇒ identical greedy episodes), validated against b45's own non-abandoned rows. Gate **≥40x** per lane. **No abandon gate**, so every row is full length and pooling becomes exact |
