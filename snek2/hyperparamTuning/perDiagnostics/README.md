...
| `vec_wave_sweep.py` | `<policy> --configs procs[:width[:intraop]] ... [--checkpoints N] [--episodes N] [--repeat N] [--spread] [--out f.json]` | **which `VEC_WAVE_PROCS` / `VEC_EVAL_WIDTH` / TF-thread config gives a host its highest eval throughput** — episodes/s, wall clock, CPU idle, min `MemAvailable` and peak summed shard RSS per config, over one fixed explicit checkpoint set. Linux only (the samplers read `/proc`). Runs everything under a `bench-vecsweep` symlink so a sweep cannot overwrite the measured arm's chart, and reads the episodes it actually measured out of the merged file rather than assuming the plan. Produced the desktop's 16-shard + `intraop=1` operating point |
| `eval_device_split.py` | `[--width N] [--repeats N] [--split-only] [--batches N ...] [--verify <policy-dir> <step>]` | **whether a GPU could take any of the eval's time, and whether it computes the policy correctly.** Splits a `vec_eval` step into the numpy env/observation build against the TensorFlow policy call, so `1/(1 - policy_share)` is the Amdahl ceiling on any accelerator (**measured: policy 8.2%, ceiling 1.09x**), then times the policy call per visible device. `--verify` is the important half: it checks the greedy policy against `argmax` over its own Q-values and reports the Q-gap where they differ. **MPS fails it — 23 of 64 states wrong by a median 0.64 in Q**, which turned four hall-of-fame champions from 97-98% perfect into 0.0% with no error and a faster wall clock. Run it before trusting any new device, accelerator build or TF version |
| `sef_common_footing.py` | `--new <evals.json> --old <evals.json> [--new-glob G --old-glob G] [--cap STEP]` | **`strong_eval_fraction` across the 2026-08-19 10→20 episode graph boundary**, which the primary metric does *not* survive. `sef` is a threshold-crossing fraction, so it rewards noise: at a true rate of 0.55 a 10-episode arm posts **5.3x** the `sef` of an identical 20-episode arm, and the bias vanishes above ~0.8, so no point estimate resolves it. Puts a 20-episode arm on the 10-episode footing exactly — `X ~ Hypergeometric(20, k, 10)`, strong iff `X ≥ 8` — with no simulation. Pairs by seed off the filename and prints the raw comparison alongside, so the artefact is visible. On `b46a` vs `b38` it moved −11.3 pp to **−4.8 pp**, sign test unchanged at 0 of 4 |
