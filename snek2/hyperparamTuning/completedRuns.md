...
**§§§ marks arms measured by the vectorised engine, whose `measured` column has no gate at all** —
`vec_wave.py` is flat and ungated (`min_achievable: null`), so every row is full length and the figure is a
plain pooled rate rather than `pooled_equal_effort` or the graph-100% tier. That is a **third** gate era
after "none through 13" and "90/95 from 14": read the payload, never the batch number. The same marker also
says the arm's graph ran **100-episode** self-evals, which lowers `best perfect-30` and `sef` systematically
against every earlier arm — so those two columns understate a §§§ row. Correct `sef` with
[`perDiagnostics/sef_common_footing.py`](perDiagnostics/sef_common_footing.py); `best perfect-30` needs the
same hypergeometric refooting, worth ~2 pp at 100-vs-10 episodes.
...
|---|---|---|---|---|---|---|---|
| `b46d-c51atoms201seed2` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****201 atoms** (vs 51)** | 3.00M | 98.0% @210k /100 | 98/96/96 | 89.97% | 90.9% | ‡‡‡ § §§§ **the batch's best arm and best checkpoint of all sixteen.** Its one ≥98%/100 row re-measured **95.4%/500** — the batch ceiling, and 3.6 pp under `b29b`'s 99.0% record. Highest `best perfect-30` of the batch despite the 100-episode graph that suppresses it. Wave verdict is still a null: +1.9 mean pp |
| `b46c-c51atoms21seed3` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****21 atoms** (vs 51)** | 3.00M | 98.0% @249k /100 | 98/93/92 | 89.11% | 79.8% | ‡‡‡ § §§§ one ≥98%/100 row, re-measured **94.8%/500** — second best of the batch. Only **9** rows selected, so its pooled is a thin, upward-censored pool and not comparable with wave 1/2 |
| `b46a-c51batch512seed4` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****batch 512** (vs 128)** | 3.00M | 98.0% @409k /100 | 98/93/90 | 75.46% | 82.2% | ‡‡‡ § §§§ one ≥98%/100 row, re-measured **91.8%/500** — the batch's largest winner's curse at −6.2 pp, and the only ≥98% selected on a *20*-episode graph. Lowest pooled of the batch |
| `b46d-c51atoms201seed4` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****201 atoms** (vs 51)** | 3.00M | 94.0% @191k /100 | 94/94/94 | 88.69% | 81.7% | ‡‡‡ § §§§ +1.5 mean pp. Three rows within 0.5 pp of each other at the top — a flat, consistent region with no candidate |
| `b46d-c51atoms201seed1` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****201 atoms** (vs 51)** | 3.00M | 97.0% @560k /100 | 97/96/87 | 88.00% | 84.1% | ‡‡‡ § §§§ **the wave's only losing seed, −4.7 mean pp**, and it ended in a deep late drawdown: trailing 87.7 at 2.95M, **56.0** at the cap, dipping to 11-20% over 2995-2999k. `zero_since` is `null`, so a drawdown not a death |
| `b46c-c51atoms21seed1` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****21 atoms** (vs 51)** | 3.00M | 97.0% @289k /100 | 97/97/95 | 88.27% | 83.6% | ‡‡‡ § §§§ **the wave's *worst* arm on the graph, −4.8 mean pp**, and the wave's highest `best perfect-30` at the same time — the two columns disagree sharply on this arm. No ≥98% row |
| `b46b-c51softtgtseed2` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****soft target** τ=0.005, period 1** | 3.00M | 97.0% @198k /100 | 97/97/95 | 87.10% | 86.5% | ‡‡‡ § §§§ highest pooled of wave 2 and its best `best perfect-30`; no candidate. This arm is mid-batch on the graph era — 20-episode evals to ~821k, 100 after |
| `b46a-c51batch512seed3` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****batch 512** (vs 128)** | 3.00M | 96.0% @626k /100 | 96/95/94 | 85.76% | 89.5% | ‡‡‡ § §§§ **the one arm of wave 1 ahead on every metric that matters**, and the wave was still a −2.5 pp null. No ≥98% row |
| `b46c-c51atoms21seed4` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****21 atoms** (vs 51)** | 3.00M | 96.0% @290k /100 | 96/96/96 | 89.14% | 82.6% | ‡‡‡ § §§§ three top rows identical at 96% — the flattest strong region in the batch. Highest pooled of any b46 arm, on 28 rows |
| `b46a-c51batch512seed2` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****batch 512** (vs 128)** | 3.00M | 95.0% @222k /100 | 95/94/94 | 82.90% | 83.0% | ‡‡‡ § §§§ the wave's worst on mean perfect (47.6, lowest of all sixteen) yet mid-pack on pooled — the split that makes `measured` a within-arm check, not a ranking |
| `b46c-c51atoms21seed2` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****21 atoms** (vs 51)** | 3.00M | 95.0% @76k /100 | 95/91/91 | 87.00% | 75.7% | ‡‡‡ § §§§ **only 8 rows selected**, the thinnest pool of the batch, and its best is at 76k — an early peak it never returned to. −1.8 mean pp on the lowest absolute mean of the wave (49.2) |
| `b46b-c51softtgtseed1` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****soft target** τ=0.005, period 1** | 3.00M | 94.0% @321k /100 | 94/93/92 | 80.59% | 75.8% | ‡‡‡ § §§§ **the widest deficit of all sixteen arms, −9.0 mean pp**, and the lowest `best perfect-30` of the batch. Mid-batch graph era |
| `b46b-c51softtgtseed4` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****soft target** τ=0.005, period 1** | 3.00M | 92.0% @477k /100 | 92/91/91 | 81.74% | 78.7% | ‡‡‡ § §§§ −0.4 mean pp, i.e. level, and no candidate. Mid-batch graph era |
| `b46b-c51softtgtseed3` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****soft target** τ=0.005, period 1** | 3.00M | 92.0% @103k /100 | 92/90/89 | 78.86% | 78.5% | ‡‡‡ § §§§ best row at 103k, i.e. in the first 3.4% of the run, and never bettered over the remaining 2.9M steps. Mid-batch graph era |
| `b46d-c51atoms201seed3` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****201 atoms** (vs 51)** | 3.00M | 90.0% @927k /100 | 90/89/89 | 85.57% | 82.5% | ‡‡‡ § §§§ +1.1 mean pp with the batch's **weakest best checkpoint** — only **7** rows cleared selection. Highest trailing of the wave (89.1) on the lowest ceiling |
| `b46a-c51batch512seed1` ‡‡‡ § §§§ | **c51** `fc 320`, Adam ε 3.125e-4, `lr 1e-4`, IS off, kl priority — ****batch 512** (vs 128)** | 3.00M | 90.0% @342k /100 | 90/89/88 | 77.58% | 81.2% | ‡‡‡ § §§§ **−4.2 mean pp against `b38a`, the strongest control arm** — the widest deficit of wave 1, though not of the batch (`b46b` seed 1's −9.0 is). No row above 90% |
...
| `b41c-b29repro-seed3` ‡‡‡ § | **fc 320**, chase-safe shaping `c=0.10` (gate 75), IS off, td_error — **`b29`'s config re-run verbatim on the same seed** | 2.00M | **98.0%** @1279k /500 | 100/100/99% | 90.61% /eq | 97.7% | ‡‡‡ § **the re-run's best arm, and `b29` called this seed third of four.** 38 rows ≥98%/100 against `b29c`'s 9, and **1 held ≥98%/500** where `b29c` held none. Highest pooled of either batch. See [the process-noise floor](findings.md#-the-process-noise-floor-measured-the-config-reproduces-the-champion-does-not-b41-vs-b29-2026-08-27) |
| `b41d-b29repro-seed4` ‡‡‡ § | **fc 320**, chase-safe shaping `c=0.10` (gate 75), IS off, td_error — **`b29`'s config re-run verbatim on the same seed** | 2.00M | 97.4% @1645k /421 *trunc* | 100/99/99% | 85.37% /eq | 97.7% | ‡‡‡ § 15 rows ≥98%/100 against `b29d`'s **1**; **0 held ≥98%/500**, best partial 97.4% abandoned at 421 ep. `b29` called seed 4 its weak seed — it is second-best here |
| `b41a-b29repro-seed1` ‡‡‡ § | **fc 320**, chase-safe shaping `c=0.10` (gate 75), IS off, td_error — **`b29`'s config re-run verbatim on the same seed** | 2.00M | 98.0% @1691k /100 | 98/97/96% | 82.11% /eq | 91.7% | ‡‡‡ § **1** row ≥98%/100 against `b29a`'s **59**, and 0 held ≥98%/500 against `b29a`'s 3. Also the only arm materially behind on the graph (mean perfect −10.1, `sef` −20.6) — the worst per-seed swing in the pair and the number behind the ~10 pp resolution floor |
| `b41b-b29repro-seed2` ‡‡‡ § | **fc 320**, chase-safe shaping `c=0.10` (gate 75), IS off, td_error — **`b29`'s config re-run verbatim on the same seed** | 2.00M | 97.0% @920k /100 | 97/97/97% | 83.81% /eq | 96.0% | ‡‡‡ § **the record seed, and it produced nothing.** `b29b` on this seed gave **64** rows ≥98%/100, an 18-checkpoint ≥98%/500 band and the 99.0%/500 project record; here the best close-out row is 97.0%/100, **0** rows reached ≥98%, and **no HOF stage ran because nothing qualified**. Graph metrics reproduce fine (mean perfect **+1.1**, `best30` −1.3), which is what makes it a clean result rather than a broken arm: **the record was a property of the run** |
| `b29b-chase10g75seed2` ‡‡‡ § | **fc 320**, chase-safe shaping `c=0.10` **(gate 75)**, IS off, td_error | 2.00M | **99.0%** @1447k /500 | 100% | 87.14% /eq §§ | 97.3% | ‡‡‡ § **the record until 2026-08-20** (99.0%, 495/500, CI 97.7-99.6) — passed by `b43b` @1661k (99.6%) and `b44b` @1886k (500/500), both *continuations* of these same weights at a lower rate — though **none of those figures survives a fresh 1000-episode measurement**, which puts the best of them at 99.0% and the four best together at 98.40%. See [the winner's-curse result](findings.md#-the-winners-curse-measured-four-selected-champions-all-fell-and-the-500500-did-not-reproduce-2026-08-20). Head of an **18-checkpoint ≥98%/500 band** (1446k-1529k) — a *region*, where b24's records are isolated points. The 99.0 vs 98.0 lead over b24 is inside the 500-ep CI; the region is not. **Promoted to [`../hallOfFame/`](../hallOfFame/HOF.md) 2026-08-16**, copy verified 98/100. See [Batches 28-29](#batches-28-29--chase-safe-dose-and-gate-the-gate-is-the-lever-and-gate-75-produces-a-record-region) |
...
| `b24d-fc320noisseed4` ‡‡‡ § | **fc 320**, IS off (`SNEK_IS_WEIGHTS=0`), td_error | 3.00M | **98.0%** @1342k /500 | 99.0% | 85.97% /eq §§ | 96.7% | ‡‡‡ § **THE NEW RECORD** — 98.0% over 500 fresh episodes (490/500, CI [96.4,98.9]), and it *rose* on re-measurement (97.0/100 → 98.0/500), the genuine-region signature. 3 of its 51 ≥97%/100 checkpoints held ≥97%/500 (early, 0.9-1.36M). In [`../hallOfFame/`](../hallOfFame/HOF.md) |
| `b24b-fc320noisseed2` ‡‡‡ § | **fc 320**, IS off, td_error | 3.00M | **98.0%** @2860k /500 | 99.0% | 88.84% /eq §§ | **96.7%** | ‡‡‡ § **new record (tied)** — 98.0%/500 (490/500, CI [96.4,98.9]); 3 of 59 ≥97%/100 checkpoints held ≥97%/500, all late (2.74-2.86M). In [`../hallOfFame/`](../hallOfFame/HOF.md) |
...
| `b35c-chase10g40seed3` ‡‡‡ § | **fc 320**, chase-safe shaping `c=0.10` **(gate 40)**, IS off, td_error | 2.00M | 100% @1166k /100 | 100% | **90.7%** /eq §§ | 97.7% | ‡‡‡ § best of b35 (gate 40) — **highest pooled of any shaped batch**. HOF-500 still running at check time; the other three seeds held 0 |
| `b35a-chase10g40seed1` ‡‡‡ § | **fc 320**, chase-safe shaping `c=0.10` (gate 40), IS off, td_error | 2.00M | 96.2% @1409k /319 *trunc* | 99.0% | 88.6% /eq §§ | 96.0% | ‡‡‡ § 2nd of b35; **0 held**, best partial 96.2% (319 ep, ab.). Gate 40 is null — the sweet spot is isolated at 75 |
| `b35d-chase10g40seed4` ‡‡‡ § | **fc 320**, chase-safe shaping `c=0.10` (gate 40), IS off, td_error | 2.00M | 97.0% @1480k /367 *trunc* | 99.0% | 87.6% /eq §§ | 96.7% | ‡‡‡ § 3rd of b35; **0 held**, best partial 97.0% (367 ep, ab.) |
| `b35b-chase10g40seed2` ‡‡‡ § | **fc 320**, chase-safe shaping `c=0.10` (gate 40), IS off, td_error | 2.00M | 96.5% @1353k /310 *trunc* | 98.0% | 85.9% /eq §§ | 94.7% | ‡‡‡ § weakest of b35; **0 held**, best partial 96.5% (310 ep, ab.) |
...
| `b36a-c51fc320seed1` ‡‡‡ § | **c51** `fc 320`, 51 atoms over `[-5,120]`, **Adam ε 1.5e-4**, `lr 1e-4`, IS off, kl priority | 2.02M | **97.0%** @550k /100 | 94.1% | 75.36% /eq §§ | 84.0% | ‡‡‡ § best checkpoint of both C51 batches, and the only one above 96. `sef` 23.2%. **No ≥98%/100, so no HOF-500** — same for all 8 arms here |
| `b36d-c51fc320seed4` ‡‡‡ § | **c51** `fc 320`, 51 atoms over `[-5,120]`, **Adam ε 1.5e-4**, `lr 1e-4`, IS off, kl priority | 1.87M | 95.0% @356k /100 | 95.0% | **80.19%** /eq §§ | **86.7%** | ‡‡‡ § highest pooled of any C51 arm, and top of b36 on best-30. `sef` 17.4% — the batch's lowest, so pooled and `sef` disagree |
| `b36b-c51fc320seed2` ‡‡‡ § | **c51** `fc 320`, 51 atoms over `[-5,120]`, **Adam ε 1.5e-4**, `lr 1e-4`, IS off, kl priority | 1.97M | 94.0% @305k /100 | 93.5% | 76.70% /eq §§ | 86.0% | ‡‡‡ § `sef` 24.7%, highest of b36. Stopped at 2M by choice, not collapse — trailing held 91.1 |
| `b36c-c51fc320seed3` ‡‡‡ § | **c51** `fc 320`, 51 atoms over `[-5,120]`, **Adam ε 1.5e-4**, `lr 1e-4`, IS off, kl priority | 2.01M | 91.6% @471k /83 *trunc* | 91.3% | 74.77% /eq §§ | 84.7% | ‡‡‡ § weakest of b36 on every column; its best row was abandoned at 83 episodes so the figure is noisier |
| `b38a-c51fc320eps3125seed1` ‡‡‡ § | **c51** `fc 320`, 51 atoms, **Adam ε 3.125e-4**, `lr 1e-4`, IS off, kl priority | 3.00M | 96.0% @2355k /100 | 96.0% | 78.51% /eq §§ | 84.3% | ‡‡‡ § **the only arm of eight whose best checkpoint is late** (2355k, past b36's horizon) and the only one whose pooled *rose* past 2M (77.99→78.51). `sef` 31.0%, highest of either batch |
| `b38b-c51fc320eps3125seed2` ‡‡‡ § | **c51** `fc 320`, 51 atoms, **Adam ε 3.125e-4**, `lr 1e-4`, IS off, kl priority | 3.00M | 95.0% @284k /100 | 94.2% | 71.79% /eq §§ | **88.3%** | ‡‡‡ § highest best-30 of either batch on the **lowest** pooled — the clearest single case that best-30 and pooled measure different things. Trailing fell 81.2→74.6 over its last 700k steps |
| `b38c-c51fc320eps3125seed3` ‡‡‡ § | **c51** `fc 320`, 51 atoms, **Adam ε 3.125e-4**, `lr 1e-4`, IS off, kl priority | 3.00M | 95.0% @290k /100 | 93.6% | 72.53% /eq §§ | 80.0% | ‡‡‡ § lowest best-30 of either batch (80.0) yet the highest trailing at cap (94.1). `sef` 18.6% |
| `b38d-c51fc320eps3125seed4` ‡‡‡ § | **c51** `fc 320`, 51 atoms, **Adam ε 3.125e-4**, `lr 1e-4`, IS off, kl priority | 3.00M | 93.4% @557k /91 *trunc* | 92.5% | 72.66% /eq §§ | 87.3% | ‡‡‡ § no full-length row survived the gate at all; `sef` 22.2%. Pooled fell 74.55→72.66 past 2M |
| `b40a-chasefree10g75seed1` ‡‡‡ § | **fc 320**, chase-safe `c=0.10` (gate 75) **+ free-space Φ = 1/(open regions)**, IS off, td_error | 2.00M | **100.0%** @1562k /100 | 100% | 88.28% /eq §§ | 96.3% | ‡‡‡ § **a flawless 100/100 close-out row** — but 0 of its 16 ≥98%/100 candidates held at 500 (best 96.3% at 294 ep, ab.), which is the attrition this file now expects. `sef` 45.0% |
| `b40c-chasefree10g75seed3` ‡‡‡ § | **fc 320**, chase-safe `c=0.10` (gate 75) **+ free-space Φ = 1/(open regions)**, IS off, td_error | 2.00M | 99.0% @1538k /100 | 99.0% | 89.11% /eq §§ | 95.3% | ‡‡‡ § 9 candidates, 0 held (best 95.6% at 251 ep, ab.). `sef` 61.5% |
| `b40d-chasefree10g75seed4` ‡‡‡ § | **fc 320**, chase-safe `c=0.10` (gate 75) **+ free-space Φ = 1/(open regions)**, IS off, td_error | 2.00M | 99.0% @1957k /100 | 99.0% | 85.68% /eq §§ | 94.0% | ‡‡‡ § the weak seed of b40, as every batch of this family has one. 2 candidates, 0 held. `sef` 56.8% |
| `b37b-chase10g75seed6` ‡‡‡ § | **fc 320**, chase-safe `c=0.10` (gate 75), IS off, td_error — **b29's config, seeds 5-8** | 2.00M | 99.0% @1343k /100 | 99.0% | **90.50%** /eq §§ | 97.7% | ‡‡‡ § **highest pooled equal-effort of the whole chase-safe family (90.50)** — and **0 of its 43 ≥98%/100 candidates held at 500** (best 97.0% at 361 ep, ab.). The single clearest case that pooled and the record tier are different metrics |
| `b37c-chase10g75seed7` ‡‡‡ § | **fc 320**, chase-safe `c=0.10` (gate 75), IS off, td_error — **b29's config, seeds 5-8** | 2.00M | 99.0% @879k /100 | 99.0% | 87.88% /eq §§ | 97.0% | ‡‡‡ § highest `sef` of b37 (58.6%); 16 candidates, 0 held (best 96.9% at 357 ep, ab.). Its best checkpoint is early, 879k |
| `b37a-chase10g75seed5` ‡‡‡ § | **fc 320**, chase-safe `c=0.10` (gate 75), IS off, td_error — **b29's config, seeds 5-8** | 2.00M | 97.0% @1495k /100 | 97.0% | 82.19% /eq §§ | 91.3% | ‡‡‡ § **no ≥98%/100 checkpoint at all** — one of the two b37 seeds that contributed nothing, mirroring b29's own 2-of-4 split |
| `b37d-chase10g75seed8` ‡‡‡ § | **fc 320**, chase-safe `c=0.10` (gate 75), IS off, td_error — **b29's config, seeds 5-8** | 2.00M | 93.4% @1393k /91 *trunc* | 93.4% | 80.72% /eq §§ | 90.0% | ‡‡‡ § weakest arm of the family on pooled (80.72); no full-length row survived the gate. `sef` 40.8% |
| `b39d-c51zeroinitseed4` ‡‡ § | **c51** `fc 320`, 51 atoms over `[-5,120]`, **`C51_ZERO_INIT=1`** (E[Q]=0 at init), Adam ε 1.5e-4, `lr 1e-4`, IS off, kl priority | 3.00M | 90.9% @2913k /77 *trunc* | 90.9% | 70.57% /eq §§ | 80.7% | ‡‡ § top of b39, and **b39 has no full-length row in any arm** — all 650 close-out rows were abandoned under the 95% gate. Its best-30 peak is at 2924k, so the 3M horizon flatters it against b36's 2M and it still loses by 6 pp |
| `b39c-c51zeroinitseed3` ‡‡ § | **c51** `fc 320`, 51 atoms over `[-5,120]`, **`C51_ZERO_INIT=1`** (E[Q]=0 at init), Adam ε 1.5e-4, `lr 1e-4`, IS off, kl priority | 3.00M | 91.9% @234k /86 *trunc* | 91.9% | 70.60% /eq §§ | 77.7% | ‡‡ § highest pooled of b39 by 0.03 pp — the batch's four seeds span **0.83 pp**, against b36's 5.4, which is what a capacity ceiling looks like rather than seed noise |
| `b39a-c51zeroinitseed1` ‡‡ § | **c51** `fc 320`, 51 atoms over `[-5,120]`, **`C51_ZERO_INIT=1`** (E[Q]=0 at init), Adam ε 1.5e-4, `lr 1e-4`, IS off, kl priority | 3.00M | 89.4% @482k /66 *trunc* | 89.4% | 69.78% /eq §§ | 75.7% | ‡‡ § the mechanism arm: action gap **1.72 at 10k** against `b36a`'s **12.18**, reaching 8.90 only by ~600k, while its value level converged *faster* (half-life 202k vs 304k). `aeff` 7.0 → 26.7 @601k → 21.1 |
| `b39b-c51zeroinitseed2` ‡‡ § | **c51** `fc 320`, 51 atoms over `[-5,120]`, **`C51_ZERO_INIT=1`** (E[Q]=0 at init), Adam ε 1.5e-4, `lr 1e-4`, IS off, kl priority | 3.00M | 91.5% @758k /71 *trunc* | 91.5% | 69.77% /eq §§ | 75.7% | ‡‡ § second mechanism arm, same shape — gap 1.69 → 9.17, `aeff` 7.0 → 27.3 @631k → 20.9, half-life 163k. Lowest pooled of the batch |
| `b18b-tgt1000seed2` ‡‡‡ § | **target period 1000**, forking on | 2.40M | **97.6%** @1588k /700 | **97.0%** | 78.52% /eq §§ | 86.0% | ‡‡‡ § **THE PROJECT RECORD** — 97.6% over 700 fresh episodes (CI 96.1-98.5), beating `b17b` by +3.33 pp, p=0.0002. **The first selected high here that did not shrink** (98/100 -> 97.4%/500). A narrow peak: @1578k reads 91.6%. In [`../hallOfFame/`](../hallOfFame/HOF.md) |
...
without inventing a position for it. As of 2026-08-17 that is the **C51 pilot** (8 arms), **`b31a-d`**,
**`b32a-d`** and **`b33a-d`** — all stopped deliberately, none measured. Their training numbers are in their
own batch sections below and in [`charts.md`](charts.md); read them as within-batch comparisons only.
**`b36a-d` and `b38a-d` do have rows** as of 2026-08-17, so C51 is finally rankable against the scalar arms
— and it ranks below them: pooled **71.79-80.19** against `b24`'s 85.97-89.03 at the identical `fc 320`.
...
record checkpoints are preserved in [`../hallOfFame/`](../hallOfFame/HOF.md).
...
## Batch 46 — four c51 knobs at n=4 each: **four nulls**

**Closed 2026-08-28.** Sixteen arms, four waves of four, 3M steps each, everything else `b38`'s config
verbatim and each arm paired against the `b38` arm of its own seed. Charts:
[`charts.md`](charts.md#batch-46--four-c51-knobs-each-at-n4--complete-four-nulls-and-the-batchs-whole-spread-fits-inside-one-seeds-noise--launched-2026-08-25).
Verdict and the power analysis:
[`findings.md`](findings.md#-four-c51-knobs-four-nulls--and-b46-could-not-have-resolved-them-closed-2026-08-28).

| wave | change | mean pp Δ | seeds ahead | corrected `sef` Δ | rows | ≥98%/100 | HOF-500 | h/arm |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| 1 `b46a` | `BATCH_SIZE` 128 → 512 | −2.5 | 1 of 4 | −5.2 | 187 | 1 | 91.8% | ~14 |
| 2 `b46b` | soft target τ=0.005, period 1 | −1.7 | 2 of 4 | −5.2 | 175 | **0** | — | ~8.7 |
| 3 `b46c` | `NUM_ATOMS` 51 → 21 | −1.6 | 1 of 4 | −3.2 | 82 | 1 | 94.8% | ~8.4 |
| 4 `b46d` | `NUM_ATOMS` 51 → 201 | **−0.0** | 3 of 4 | −2.3 | 68 | 1 | **95.4%** | **18.5** |

**Three ≥98% checkpoints from sixteen arms, all three fell on re-measurement** (91.8 / 94.8 / 95.4 at 500
episodes). Batch ceiling **95.4%** against `b29b`'s 99.0% record. The four wave means span **2.5 pp**, which
is smaller than `b41`'s *mean* per-seed swing under no change at all (4.1 pp) — so the batch was
underpowered for the effects it went looking for, and its most transferable result is that fact.

**Original design rationale and the per-wave narratives follow, as written while it ran.**
...
expect zero or one. Full tables in [`charts.md`](charts.md#batch-46--four-c51-knobs-each-at-n4--complete-four-nulls-and-the-batchs-whole-spread-fits-inside-one-seeds-noise--launched-2026-08-25).
...
**Null on both metrics that matter.** Pooled ties the family (b29 87.83, b34 86.43, b35 88.20), and the ≥98%/100
distribution **16 / 63 / 9 / 2** is close to b29's own **59 / 64 / 9 / 1**. Two arms produced a flawless
**100.0%/100** row (`b40a` @1562k, `b40b` @1424k) and one checkpoint held 500 episodes — `b40b` @1513k at
**98.2%/500**, third-best /500 on record and a HOF-promotion candidate.

**The hypothesis is answered, and answered against.** The term was aimed at the
[one-piece-free-space gap](findings.md#-the-packing-property-the-records-keep-their-free-space-in-one-piece-and-it-separates-them-by-87-points)
that separates records from duds by 87 points, which is the strongest per-policy separation this project has
measured — and shaping it changed nothing. **Knowing which quantity distinguishes good policies did not make
shaping that quantity work.** Fourth PBRS term to come back null.

**Its wider consequence is about `b29`, not about free space.** With `b37` (b29's config on fresh seeds) holding
**0 of 4**, three batches with indistinguishable /100 tiers produced held counts of **21, 1 and 0** — so that
count is seed noise, and b29's 18-checkpoint band was a seed. The correction is in
[`findings.md`](findings.md#-corrected-2026-08-18-the-record-region-does-not-replicate--the-98500-count-is-seed-noise-and-pooled-is-the-only-metric-of-this-family-worth-reading).


## Batch 39 — **C51 initialised at expected Q = 0**: it loses, and through the head's capacity rather than its calibration
...
*best-30 and `sef` matched at ≤1.87M, b36's shortest horizon; pooled over each arm's whole close-out.*

**H2 confirmed, H1 falsified**, 4 of 4 seeds on both metrics. **All 650 close-out rows were abandoned under the
95% gate** — no measurable checkpoint anywhere in the batch, against 4 full-length rows in b36 and 5 in b38 — and
the pooled spread across seeds is **0.83 pp** (b36: 5.4), which reads as a ceiling rather than luck.

**The pre-registered mechanism was wrong.** Zero-init's value level converged **faster** (half-life 163-202k vs
b36a's 304k, wash-out 601-631k vs 864k) from a **larger** initial error, so calibration is not the channel. The
channel is **action separation**: b36a reaches a 12.18 action gap by 8k steps, b39 sits at 1.72 and needs ~600k
to reach 8.90, and `argmax` depends only on differences. The `aeff` path was non-monotonic exactly as
pre-registered (7.0 → ~27 @600-630k → 21 vs b36's monotone 49.6 → 21.6). Full account in
[`findings.md`](findings.md#-zero-init-loses-and-the-channel-is-action-separation-not-calibration--b39-closed-at-3m).


## Batch 37 — **`b29` replicated on fresh seeds 5-8**: the /100 band replicates, the /500 record does not

**Retrieved into `runs/` on 2026-08-18 and confirmed — the provisional figures read off the `results` branch
...
| arm | best-30 | `sef` | pooled/eq | ≥98%/100 | held ≥98%/500 |
|---|---|---|---|---|---|
| `b37b` seed6 | **97.7** | 56.0 | **90.50** | **43** | 0 — best 97.0%, ab. at 361 ep |
| `b37c` seed7 | 97.0 | **58.6** | 87.88 | 16 | 0 — best 96.9%, ab. at 357 ep |
| `b37a` seed5 | 91.3 | 49.8 | 82.19 | 0 | 0 |
| `b37d` seed8 | 90.0 | 40.8 | 80.72 | 0 | 0 |
| **group** | | | **85.32** | **59, 2 of 4 seeds** | **0 of 4** |

**Outcome 3 of the three pre-registered readings: "b29's region was lucky seeds".** The 2-of-4 split reproduces
exactly, and `b37b` posts the family's best pooled arm while holding nothing at 500 — the sharpest single
illustration that pooled and the record tier measure different things. Retrieved into `runs/` 2026-08-18; the
provisional figures read off the `results` branch the day before were correct.


## Batch 36 — C51 on `fc 320`: **the better C51 shape, and still far behind `ddqn`**
...
**The ladder's deep rung, one variable below b34.** `b29`'s config with `SNEK_CHASE_SAFE_GATE=40` — everything
else identical (`fc 320`, IS off, `td_error`, target 1000, discount 0.9975, `FORK_BRANCHES=4`, `c=0.10`, 2M cap,
seeds 1-4). `c` stays 0.10 (the calibration clamp binds there for any gate ≤ 85), so this holds the per-flip
dose constant and moves only the gate, letting the total episode dose rise ~2.5× vs gate 85. Trained and closed
out on the desktop; HOF-500 done for three of four seeds (`b35c`'s was still running at check time).

**Gate 40 is a null on the record metric — but it posts the highest pooled of any shaped batch.** Pooled
equal-effort **88.2** (88.6 / 85.9 / 90.7 / 87.6), *above* b29's 87.8, b34's 86.4 and even the b24 control's
87.9 — yet **0 of the 3 measured seeds held any ≥98%/500 checkpoint**, best partials all abandoned at 96-97%
over 310-367 episodes. So mid-game shaping produced a slightly healthier *average* board without ever reaching
the record-tier endgame. **Consolidation and the record tier are decoupled**, which is the sharpest statement of
why pooled/best-30 are the wrong readout up here.

| arm | peak trail | best-30 | `sef` | pooled (eq) | HOF-500 (≥98%/500) |
|---|---|---|---|---|---|
| `b35c` seed3 | 95.00 | 97.7% | 59.7% | **90.7** | HOF-500 running (best full 100% @1166k /100) |
| `b35a` seed1 | 95.00 | 96.0% | 47.4% | 88.6 | 0 held — best partial 96.2% @1409k (319 ep, ab.) |
| `b35d` seed4 | 95.00 | 96.7% | 61.3% | 87.6 | 0 held — best partial 97.0% @1480k (367 ep, ab.) |
| `b35b` seed2 | 95.00 | 94.7% | 62.8% | 85.9 | 0 held — best partial 96.5% @1353k (310 ep, ab.) |
| **mean** | | **96.3%** | **57.8%** | **88.2** | **0 of 3 measured (b35c pending)** |

**The verdict: gate 75 is an isolated sweet spot.** Across four gates — 85 (b27/b28/b30), 75 (b29), 70 (b34) and
40 (b35) — **only 75 produces a ≥98%/500 region**; 85, 70 and 40 are all null. So the useful window is a narrow
band around 75, not a monotone ladder and not "anything below 85". All four b35 arms were healthy throughout
(peak 95.00, no dead or zero stretch), so the term never destabilizes; too-early shaping (40) just grades the
mid-game, which lifts the average board without buying the endgame. Full conclusion:
...
project record**, promoted to [`../hallOfFame/`](../hallOfFame/HOF.md) on 2026-08-16 (rsynced off the
...
(98/100 → 97.4/500). Both are in [`../hallOfFame/`](../hallOfFame/HOF.md).
...
[`../hallOfFame/`](../hallOfFame/HOF.md#-b18b-1588000--the-first-selected-high-that-did-not-shrink)
