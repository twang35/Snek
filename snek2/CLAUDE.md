# snek2 — the TensorFlow era, frozen

**snek2 is an archive as of 2026-08-28.** It stays runnable in the `snek` conda env for A/B against
snek3, and nothing in it is edited. Active work is in [`../snek3/`](../snek3/); its instructions are
[`../snek3/CLAUDE.md`](../snek3/CLAUDE.md) and the port design is
[`../snek3/plans/pytorch-port.md`](../snek3/plans/pytorch-port.md).

Everything below moved here verbatim from the repository's root `CLAUDE.md` when snek3 was created.
It is snek2's operating manual: what to run, what not to delete, and — mostly — the incidents behind
each rule. It is loaded only when a session touches `snek2/`, which is the point of the move.

**Paths are written relative to the repository root**, as they were at the root file, so `snek2/foo`
means this directory's `foo`.
