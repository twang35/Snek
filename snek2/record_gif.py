...
`random.seed` fully determines the game: food placement is the only randomness in `Snake.py` and a
...
Environment: none of its own. `SNEK_TILE_PIXELS` is set from `--tile` before `snake_constants` is
imported, because every pixel constant is derived from it at import time.
"""
import os
import sys

# Before any pygame or snake_constants import. The dummy driver is what makes this headless
# *and* what makes it fast; audio is silenced for the reason every entry point does it.
...
# `--tile` has to reach snake_constants before it is imported, so argv is read here rather than
# in main(). Everything else is parsed normally below.
...
HOF_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'hallOfFame')
...
# truth, and it is one line to move. b44a-lowlr7-b29b @2739000 measured 98.9% over 1000 fresh
# episodes on 2026-08-28, the highest in the project.
HOF_RECORD = 'b44a-lowlr7-b29b-ckpt2739000'
...
def suppress_game_hud():
    ...
    `render()` blits the score, step and policy readouts *before* `self.all.draw()`, so the
    sprites cover them and they are illegible from about half a board on. A header composited
    outside the board replaces them. Assigning on `Snake` and not on `snake_constants` is the
    point: `from snake_constants import *` bound copies at import time, so the module the drawing
    code actually reads is this one.
    """
    Snake.SCORE_COLOR = Snake.BACKGROUND_COLOR
    Snake.STEP_COLOR = Snake.BACKGROUND_COLOR
    Snake.POLICY_COLOR = Snake.BACKGROUND_COLOR
...
def disable_display_throttles():
    ...
    scale ever changed. `PERFECT_GAME_WAIT_MS` is read through the module by `render()`, so
    setting it here does reach it.
    """
    Snake.SCORE_SLOW_THRESHOLD = 10 ** 9
    snake_constants.PERFECT_GAME_WAIT_MS = 0
...
def overlay_win_message(game):
    ...
    """
    font = game._message_font(PERFECT_TEXT, snake_constants.PERFECT_FONT_SIZE)
    image = font.render(PERFECT_TEXT, True, (0, 0, 0))
    game.screen.blit(image, image.get_rect(center=snake_constants.SCREENRECT.center))
...
def overlay_death_message(game):
    ...
    """
    if game.starved:
        text, size = 'NO FUD', snake_constants.STARVE_FONT_SIZE
    else:
        text, size = 'DED', snake_constants.DEATH_FONT_SIZE
...
class Composer:
    ...
    def compose(self, board, score, step, game_index, game_count):
        ...
        line = 'meals %2d/%d   step %4d' % (score, snake_constants.MAX_POSSIBLE_SCORE, step)
        if game_count > 1:
...
def resolve_policy(spec, step):
    ...
    The step is named explicitly rather than found with `tf.train.latest_checkpoint`, which
    returns `None` in a hall-of-fame directory — there is no `checkpoint` index file there, only
    the two `ckpt-N` files — and would then fail the restore silently.
    """
    if spec == 'hof':
        spec = HOF_RECORD
    ...
    if not steps:
        raise SystemExit('no ckpt-*.index files in %s' % directory)
    if step is None:
        step = steps[-1]
...
def play_episode(tf_env, env, policy_action):
    """One greedy episode, captured. Returns `(boards, meta, won)`.
...
def main(argv):
    ...
    # Imported here, not at module scope: TensorFlow costs seconds to import, and --list and every
    # argument error above should be instant.
    import tensorflow as tf  # noqa: F401  (imported for its side effects on device config)
    ...
    env = SnakeEnvironment(discount=0.9975, display=True, limit_fps=False, policy_name='')
    tf_env = tf_py_environment.TFPyEnvironment(env)
    agent, checkpoint, _ = build_eval_agent(tf_env, env, ckpt_dir)
    checkpoint.restore(os.path.join(ckpt_dir, 'ckpt-%d' % step)).expect_partial()
    ...
    for attempt in range(args.max_games):
        ...
        random.seed(seed)
        episode_boards, episode_meta, won = play_episode(tf_env, env, policy_action)
        game = env._game
        outcome = 'PERFECT' if won else ('starved' if game.starved else 'died')
        print('  seed %d: score %d, %d steps, %s'
              % (seed, env.get_score(), game.current_step, outcome))
    ...
    out_path = args.out or os.path.join('gifs', '%s.%s' % (label.replace(' @', '-'), args.format))
